...
export async function generateSummaryByPostId({
  ...
  if (plainText.length < 100) return post;
...
export async function findPostById(
  ...
  const kvHash = await CacheService.getRaw(context, POSTS_CACHE_KEYS.syncHash(post.id));
...
export async function deletePost(
  ...
  if (post.status === "published") {
    ...
    const version = await CacheService.getVersion(context, "posts:detail");
    tasks.push(CacheService.deleteKey(context, POSTS_CACHE_KEYS.detail(version, post.slug)));
    ...
    tasks.push(CacheService.deleteKey(context, POSTS_CACHE_KEYS.syncHash(data.id)));
...
export async function startPostProcessWorkflow(
  ...
  try {
    const oldInstance = await context.env.SCHEDULED_PUBLISH_WORKFLOW.get(scheduledId);
...
/**
 * 可选：给“默认导入对象”用（不影响你现在的测试）。
 * 以后就算有人写 `import PostService from ...` 也能用。
 */
